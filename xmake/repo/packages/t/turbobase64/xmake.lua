package("turbobase64")
	...
	on_install("linux", "macos", "windows", function (package)
		...
		elseif is_arch("ppc64le") then
			table.insert(configs, "-DCMAKE_SYSTEM_PROCESSOR=ppc64le")
		elseif is_arch("riscv64") then
			table.insert(configs, "-DCMAKE_SYSTEM_PROCESSOR=riscv64")
		else
			table.insert(configs, "-DCMAKE_SYSTEM_PROCESSOR=unknown")
		end
