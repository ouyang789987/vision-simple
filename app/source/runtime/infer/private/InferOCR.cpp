...
struct vision_simple::InferOCROrtPaddleImpl::Impl
{
    ...
    cv::Mat& DetPreProcess(const cv::Mat& image) noexcept
    {
        ...
        if (chwrgb_image.rows != target_size.height || chwrgb_image.cols != target_size.width)
            chwrgb_image = cv::Mat::zeros(target_size, CV_8UC3);
        vision_helper.HWC2CHW_BGR2RGB<uint8_t>(padded_img, chwrgb_image);
        if (preprocessed_image.rows != target_size.height || preprocessed_image.cols != target_size.width)
    ...
    std::vector<cv::Rect> DetPostProcess(const Ort::Value& output_tensor,
                                         const cv::Size input_image_size, const cv::Size original_image_size,
                                         ...
                                         int kernel_size = 8) noexcept
    {
        ...
        auto img = cv::Mat{
            static_cast<int>(output_shape[2]), static_cast<int>(output_shape[3]),CV_32FC1, (void*)(output_ptr)
        };
    ...
    Ort::Value RecPreProcess(const cv::Mat& image, const cv::Rect& box, int fixed_height = 48)
    {
        ...
        const auto output_image_size_bytes = static_cast<size_t>(output_size.width) * output_size.height * 3
            * sizeof(float);
        int64_t tensor_shape[4] = {1, 3, output_size.height, output_size.width};
        auto tensor = Ort::Value::CreateTensor<float>(rec_allocator, tensor_shape, 4);
        ...
        box_image.copyTo(resized_image(cv::Rect(0, 0, box_image.cols, box_image.rows)));
    ...
    static auto FindMaxValueIndex(const std::span<const float> vec) -> std::pair<float, size_t>
    {
     ...
     * @return 
     */
    std::vector<std::pair<std::string, float>> RecPostProcess(const std::span<const float> output,
                                                              ...
                                                              float confidence_threshold = 0.5f)
    {
        ...
        for (auto i{0}; i < output_shape[0]; ++i)
        {
            ...
            auto confidence = !scores.empty()
                                  ? std::accumulate(scores.cbegin(), scores.cend(), 0.0f) /
                  ...
                  float confidence_threshold) noexcept
    {
        ...
        auto input_size_bytes = static_cast<unsigned long long>(input_image.channels())
        ...
        auto boxes = DetPostProcess(output_tensor, input_image_size, original_image_size);
        ...
        for (size_t i = 0; i < boxes.size(); ++i)
        {
            ...
            auto rec_output_shape = rec_output_tensor.GetTensorTypeAndShapeInfo().GetShape();
            const auto rec_output_tensor_size = std::accumulate(rec_output_shape.begin(), rec_output_shape.end(), 1llu,
                                                                std::multiplies());
            auto span = std::span(rec_output_tensor.GetTensorData<float>(), rec_output_tensor_size);
            auto lines = RecPostProcess(span, rec_output_shape, confidence_threshold);
            if (!lines.empty())
                frame_result.results.emplace_back(box, lines[0].second, std::move(lines[0].first));
        }
        return frame_result;
    }
};
