...
int main() {
  ...
  std::string str{reinterpret_cast<const char*>(char_dict_data->data.get()),
                  char_dict_data->size};
  std::string wstr{reinterpret_cast<const char*>(char_dict_data->data.get()),
                   char_dict_data->size};
  ...
  auto infer_ocr = vision_simple::InferOCR::Create(
      **infer_ctx, char_dict, det_data->span(), rec_data->span(),
