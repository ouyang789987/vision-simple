...
namespace vision_simple
{
    ...
    class HTTPServerImpl : public HTTPServer
    {
        ...
        GetYOLOModel(const std::string& name)
        {
            ...
            if (auto it = std::ranges::find_if(config.model_config().yolo, [&name](const auto& item)
            {
            ...
            }); it != config.model_config().yolo.end())
            {
                ...
                auto& device_str = options_.OptionOrPut(HTTPSERVER_OPT_KEY_INFER_DEVICE,
            ...
            return MK_VSERROR(VisionSimpleErrorCode::kModelError, "unable to find model: " + name);
        }
        ...
        {
            {
                ...
                if (auto it = ocr_models_cache_.find(name); it != ocr_models_cache_.end())return *it->second;
            }
            ...
            if (!config_result)return std::unexpected(std::move(config_result.error()));
            auto& config = config_result->get();
            if (auto it = std::ranges::find_if(config.model_config().ocr, [&name](const auto& item)
            {
                return name == item.name;
            }); it != config.model_config().ocr.end())
            {
                const auto& model_info = *it;
                auto model_type_opt = magic_enum::enum_cast<OCRModelType>(model_info.version);
                if (!model_type_opt)
                ...
                auto infer_ocr_result = InferOCR::Create(*infer_context_, model_info.char_dict_path,
                                                         model_info.det_path,
    ...
    public:
        ...
        int HandleInferOCR(const HttpContextPtr& ctx) noexcept
        {
            ...
            for (auto& result : all_results)
            {
                ...
                {
                    lines.emplace_back(OCRLine{
                        std::move(ocr_result.line),
...
{
    auto infer_fw_str = options.OptionOrPut(HTTPSERVER_OPT_KEY_INFER_FRAMEWORK, HTTPSERVER_OPT_DEFVAL_INFER_FRAMEWORK);
    auto infer_ep_str = options.OptionOrPut(HTTPSERVER_OPT_KEY_INFER_EP, HTTPSERVER_OPT_DEFVAL_INFER_EP);
